...
class NeuralNetwork:
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        # 初始化weight和bias
        self.w1 = linear(self.input_dim, self.hidden_dim) #512*2
        self.bias1 = bias(self.input_dim, self.hidden_dim) #512*2
        self.w2 = linear(self.hidden_dim, self.output_dim) #3*512
        # self.bias2 = bias(self.input_dim, self.hidden_dim)
    ...
    def forward(self, X):
        ...
        self.z2 = np.dot(self.a1, self.w2.T) # 第二層的輸出 (32*512, 512*3)=32*3
        self.a2 = self.softmax(self.z2) # 第二層的softmax輸出 32*3
        return self.z2
    ...
    def backward(self, X, y, learning_rate, batch_size):
        ...
        delta2 = error
        delta1 = np.dot(delta2, self.w2)*self.relu_der(self.z1)
        ...
        return self.w1, self.w2, self.bias1
    
TwoNNmodel=NeuralNetwork(2,512,3)

loss_fn = nn.CrossEntropyLoss()

def train(dataloader, wt1, wt2, loss_fn, b):
    ...
    for X, y in tqdm(dataloader):
        ...
        pred = TwoNNmodel.forward(X_np)
        # pred_list1.apend(pred)
        one_hot_y=np.eye(3)[y]

        # Backpropagation
        weight1, weight2, b= TwoNNmodel.backward(X_np, one_hot_y, 1e-3, X_np.shape[0])
...
def test(dataloader, wt1, wt2, loss_fn, b):
    num_batches = len(dataloader)
    size = len(dataloader.dataset)
    epoch_loss = 0
    correct = 0
...
for epoch in range(epochs):
    train_loss, train_acc = train(train_dataloader, TwoNNmodel.w1, TwoNNmodel.w2, loss_fn, TwoNNmodel.bias1)
    test_loss, test_acc = test(test_dataloader, TwoNNmodel.w1, TwoNNmodel.w2, loss_fn, TwoNNmodel.bias1)
...
d1=decision_region(TwoNNmodel, train_array, label_mat1)
d2=decision_region(TwoNNmodel, test_array, label_mat2)
